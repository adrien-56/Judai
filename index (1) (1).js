      ...
      if (
        ...
        if (message.embeds.length > 0) {
          ...
          if (embed.title.includes("has appeared")) {
            ...
            if (!imageUrl) return;
            const endpoint = "http://dono-03.danbot.host:7948/predict";
            try {
              ...
              if (response.data && response.data.name) {
                ...
                let logMsg = `AI prediction: ${response.data.name}` + (ms !== undefined ? ` (${ms}ms)` : "");
                console.log(logMsg);
                // Log to Discord
                const embedLog = new EmbedBuilder()
                  .setTitle("AI Prediction")
                  .setDescription(`Predicted: **${response.data.name}**\nTime: ${ms !== undefined ? ms + "ms" : "unknown"}`)
                  .setColor("F5A623")
                  .setTimestamp();
                await message.channel.send(`<@${poketwo}> c ${response.data.name}`);
                await message.channel.send({ embeds: [embedLog] });
          ...
          message.channel.createMessageCollector({ filter, time: 5000 })
            .on('collect', async (msg) => {
              ...
              logHook([questEmbed]);
              log(`Quest completed: ${msg.content.substring(0, 50)}...`.purple);
            });
        } else if (
